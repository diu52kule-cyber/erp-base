...
import { buildEInvoiceJson } from '@/lib/invoice/govt';
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (invoice.doc_type !== 'invoice') return NextResponse.json({ error: 'e-Invoice applies to tax invoices only' }, { status: 400 });
  ...
  return new NextResponse(JSON.stringify(json, null, 2), {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${invoice.invoice_number}-einvoice.json"`,
    },
  });
}
