...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const items = (invoice.invoice_items ?? []).map((item: any) => ({
    ...
    gst_amount: item.gst_amount, amount: item.amount, discount_amount: item.discount_amount, hsn_code: item.hsn_code,
  }));

  const invData = {
    doc_type: docType, currency,
    invoice_number: invoice.invoice_number, reference_no: invoice.reference_no, status: invoice.status,
    ...
    subtotal: invoice.subtotal, discount_amount: invoice.discount_amount, discount_type: invoice.discount_type,
    discount_value: invoice.discount_value, round_off: invoice.round_off, amount_paid: invoice.amount_paid,
    gst_amount: invoice.gst_amount, total: invoice.total,
    notes: invoice.notes, terms: invoice.terms, place_of_supply: invoice.place_of_supply,
  ...
  const total  = fmtMoney(invoice.total, currency);
