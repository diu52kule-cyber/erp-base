...
export default function Sidebar({
  ...
  return (
    <aside className={`relative flex h-screen shrink-0 flex-col border-r border-neutral-200 bg-white transition-[width] duration-200 ${collapsed ? 'w-[68px]' : 'w-64'}`}
      ...
      <button onClick={toggleCollapsed} aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
        className="absolute -right-3 top-1/2 z-20 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-neutral-200 bg-white text-neutral-500 shadow-sm hover:bg-neutral-50 hover:text-neutral-900">
      ...
      {/* Brand + bell */}
      <div className={`flex items-center gap-2 border-b border-neutral-100 px-4 py-3.5 ${collapsed ? 'justify-center px-0' : ''}`}>
        <Logo />
        {!collapsed && <span className="flex-1 text-[15px] font-semibold tracking-tight text-neutral-900">{APP_NAME}</span>}
        {!collapsed && <NotificationBell />}
      </div>

      {/* Workspace card */}
      <div className={`pt-3 ${collapsed ? 'px-2' : 'px-3'}`}>
        <div className={`flex items-center gap-2.5 rounded-lg border border-neutral-200 bg-neutral-50 ${collapsed ? 'justify-center p-2' : 'px-3 py-2'}`}
      ...
      <div className="space-y-1.5 border-t border-neutral-100 p-3">
        ...
        <Link href="/dashboard/settings/preferences" title={collapsed ? 'Settings' : undefined}
          className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900 ${collapsed ? 'justify-center px-0' : ''}`}>
